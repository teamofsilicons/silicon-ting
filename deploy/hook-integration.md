...
Local regression tests cover authorization, recovery, expiry, receiver isolation and lifecycle fences. [Nine live acceptance groups](hook-scopes-rollout-014.json) also passed on backend 0.1.4 using real IAM authority in one isolated environment. Both Carbon and Silicon acquired receivers using only Hook's selector and IAM-issued audience headers. Required muted sends reached an actual Ting hook and scoped watch; ACK semantics, producer dedupe, opt-out, grant revocation and re-enrollment passed. Six test tings were accepted; no production notifications, grants or preferences were changed.

Coordinated clean invalidated both fresh capabilities before their original expiries. The task-owned environment was then deleted with all participant receipts ready and no pending operation. Temporary credential staging was removed. Rotation and restore remain covered by local regressions, not additional live claims. [Verification](verification.md) separately records real late-login recovery and release checks. These protocol tests do not establish production scope approval or a complete Hook worker/browser adapter flow.
