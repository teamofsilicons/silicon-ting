...
| Backend | [All three backend workflow jobs and the SSM deployment passed](backend-019-deployment-results.json). The archive checksum and embedded commit were verified; public health reports 0.1.9. Server code is unchanged from 0.1.8 apart from its version. |
| Frontend | [Vercel production promotion and 16 public resource comparisons passed](frontend-rollout-019.json). Both domains serve the tested build, the on-demand daemon docs and 0.1.9 installers. A first deployment omitted the telemetry build inputs and was replaced minutes later; the promoted bundle includes both telemetry tables. |
| Public boundaries | [45 live HTTP/CORS checks passed](public-browser-smoke-019-results.json) across backend and frontend proxy, including DM, Interface and Hook origins, without notification writes. |
